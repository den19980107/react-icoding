...
router.get('/showStudentRFMInClass/:classID', ensureAuthenticated, function (req, res) {
	StudebtTakeCourse.find({ classID: req.params.classID }, async function (err, records) {
		...
		for (let i = 0; i < studentsID.length; i++) {
			...
			let userData = await User.findById(studentId, function (err, userData) {
				return userData
			})
		...
		studnetsLoginData.forEach(studnetLoginData => {
			...
			let studentTimeSpend = {
				...
				userName: studnetLoginData.userData.name,
				F: 0,
				R: 0
			}
			...
			for (let i = 0; i < loginData.length; i++) {
				...
				if (startTime && endTime && endTime > startTime) {
					studentTimeSpend.M += (endTime - startTime) / 1000 * inclassTime / totalInTime;
					studentTimeSpend.F++

					if (endTime > studentTimeSpend.R) {
						studentTimeSpend.R = endTime
					}
					startTime = undefined;
					endTime = undefined;
					isStart = false;
